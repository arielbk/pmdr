...
**Deviations:** Marked `needs-review` because the slice has a human checkpoint and the specified feedback loop is manual GUI behavior; structural build and unit tests pass.
**Handoff:** The controller now accepts injectable `positionStore` and `screenProvider` for tests; production uses `NSScreen.main ?? NSScreen.screens.first` as the active display. `saveCurrentPosition()` records against the display intersecting the panel frame when available, falling back to the active display provider. `xcodegen generate` succeeds. `xcodebuild build-for-testing -scheme pmdr-menubar -destination 'platform=macOS' -derivedDataPath DerivedData CODE_SIGNING_ALLOWED=NO` succeeds. Direct `xcrun xctest DerivedData/Build/Products/Debug/pmdr-menubarTests.xctest` passes 76 tests, with the 3 pre-existing CLI integration tests skipped.
